...
const SUPA_URL   = process.env.SUPA_URL    || "https://xwennpgxsndjpbwdmcwk.supabase.co";
const SUPA_KEY   = process.env.SUPA_KEY    || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inh3ZW5ucGd4c25kanBid2RtY3drIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzY5ODc1MjksImV4cCI6MjA5MjU2MzUyOX0.LAyCQtlsF9puLftwyK67qwsYED7PksZeP3QoD0lmuTA";
  ...
  if (req.method === "POST" && u.pathname === "/stripe/webhook") {
    ...
    try {
      ...
      if (event.type === "invoice.payment_succeeded") {
        ...
        if (subId) {
          await supabasePatch("subscriptions",
      ...
      if (event.type === "invoice.payment_failed") {
        ...
        if (subId) {
          ...
          if (subs.length) {
            await supabasePatch("vendors",
              { id: `eq.${subs[0].vendor_id}` },
              { status: "paused" }
            );
          }
        }
      ...
      if (event.type === "customer.subscription.deleted") {
        ...
        await supabasePatch("subscriptions",
          { stripe_subscription_id: `eq.${obj.id}` },
          { status: "cancelled" }
        );
      }
    } catch (e) {
